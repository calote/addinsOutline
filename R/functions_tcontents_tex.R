...
func_tcontenido_Rmd = function(ficheroRmd_prin) {
  ...
  #browser()
  lrmd = str_which(fic02,"^\\\\input\\{([:graph:]*)\\}")
  crmd = str_extract(fic02[lrmd],"\\{[:graph:]*\\.tex\\}")
  crmd2 = str_replace_all(crmd,fixed("{"),"")
  crmd2 = str_replace_all(crmd2,fixed("}"),"")

  lrmda = str_which(fic02,"^\\\\include\\{([:graph:]*)\\}")
  crmda = str_extract(fic02[lrmda],"\\{[:graph:]*\\.tex\\}")
  crmd2a = str_replace_all(crmda,fixed("{"),"")
  crmd2a = str_replace_all(crmd2a,fixed("}"),"")
